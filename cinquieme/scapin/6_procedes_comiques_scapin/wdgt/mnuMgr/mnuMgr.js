...
var mnuMgr = {
	...
	init : function() {
		...
		if (tplMgr.fScreenTouch){
			...
			this.fOut.style.overflow="hidden";
		...
		vMnuOpnImg.src = "../skin/img/mnu/collapse.png";
		vMnuOpnImg.onmouseover = function(){this.src="../skin/img/mnu/collapseOver.png";}
		vMnuOpnImg.onmouseout = function(){this.src="../skin/img/mnu/collapse.png";}
		...
		vMnuClsImg.src = "../skin/img/mnu/open.png";
		vMnuClsImg.onmouseover = function(){this.src="../skin/img/mnu/openOver.png";}
		vMnuClsImg.onmouseout = function(){this.src="../skin/img/mnu/open.png";}
	...
	xBuildSub : function(pBtn) {
		if (!this.fBranches) {// Load outline.xml if needed
			...
			vAjax.send(null);
			this.fXmlMnu = vAjax.responseXML;
			if (!this.fXmlMnu) return false;
			this.fBranches = vAjax.responseXML.getElementsByTagName("b");
		}
		var vBranch;
		var vLbl = pBtn.fLbl;
		for (var i=0; i < this.fBranches.length; i++){
			...
			if (vBranch.getAttribute("u") == vLbl.id){
				...
				while (vChi){
					...
					if (vChi){
						vDpth = scPaLib.findNodes("anc:ul.mnu_sub").length + 1;
