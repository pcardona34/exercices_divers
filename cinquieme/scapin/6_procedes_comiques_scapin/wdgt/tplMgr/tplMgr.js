...
var tplMgr = {
	...
	fPlayBtnPath : "ide:tplLft/des:.playBtn",
	fToolBtnPath : "ide:tplLft/des:.toolsBtn",
...
/* === Public API =========================================================== */
	...
	init : function(){
		...
		// FireFox mobile (Fennec) is not yet compatible with the touch mode : swipe unusable etc
...
//		this.fScreenTouch = true;
//		this.fScreenSmall = true;
//		this.fScreenTouch = false;
//		this.fScreenSmall = false;
		...
		if (this.fScreenTouch){// Load the iScroll lib...
			...
			vAjax.open('GET', "../wdgt/tplMgr/iscroll.js", false);
			vAjax.send(null);
	...
	onLoad: function() {
		...
		// Set Tool & play button onclicks.
		var vPlayBtn = scPaLib.findNode(this.fPlayBtnPath);
		if(vPlayBtn) vPlayBtn.onclick=function(){
			var vUrl = tplMgr.fStore.get("courseUrl");
			if(vUrl) this.setAttribute("href", vUrl);
		}
		var vToolBtn = scPaLib.findNode(this.fToolBtnPath);
		if(vToolBtn) vToolBtn.onclick=function(){
